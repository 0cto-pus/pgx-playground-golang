...
func (productController *ProductController) GetProductById(c echo.Context) error {
	...
	if err != nil {
		 return response.ErrorMessage(c , http.StatusNotFound, err) 
	}
	return c.JSON(http.StatusOK, response.SuccessResponse(product))
}
...
func (productController *ProductController) GetAllProducts(c echo.Context) error {
	...
	if len(store) == 0 {
		...
		return c.JSON(http.StatusOK, response.SuccessResponse(allProducts))
	}
	productsWithGivenStore := productController.productService.GetAllProductsByStore(store)
	return c.JSON(http.StatusOK, response.SuccessResponse(productsWithGivenStore))
}
...
func (productController *ProductController) AddProduct(c echo.Context) error {
	...
	fmt.Println(bindErr)
	err := productController.productService.Add(req)
