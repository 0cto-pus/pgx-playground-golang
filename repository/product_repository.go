...
func (productRepository *ProductRepository) GetById(productId int64) (domain.Product, error) {
	...
	if scanErr != nil && scanErr.Error() == "no rows in result set" {
		return domain.Product{}, fmt.Errorf(fmt.Sprintf("Product not found with id %d", productId))
	}
	if scanErr != nil {
		return domain.Product{}, fmt.Errorf(fmt.Sprintf("Error while getting product with id %d", productId))
	}
...
func (productRepository *ProductRepository) DeleteById(productId int64) error {
	...
	if err != nil {
		return fmt.Errorf(fmt.Sprintf("Error while deleting product with id %d", productId))
	}
...
func (productRepository *ProductRepository) UpdatePrice(productId int64, newPrice float32) error {
	...
	if err != nil {
		return fmt.Errorf(fmt.Sprintf("Error while updating product with id : %d", productId))
	}
